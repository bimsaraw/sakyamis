      ...
      <form id="frmEnrolledStudents">
        <div class="form-row">
          ...
          <div class="form-group col-md-3">
            ...
            <input type="text" class="form-control form-control-sm" name="full_name" placeholder="Kasun" id="full_name">
          </div>
          ...
          <?php if($this->user_model->validate_permission($username,25)) { ?>
    ...
    <?php } else { ?>
      ...
      <form id="filterInquiries">
        <div class="form-row">
          ...
          <div class="form-group col-md-4">
            <label>Student Name</label>
            <input type="text" id="name" name="name" class="form-control form-control-sm" placeholder="Kasun%">
          </div>
          <div class="form-group col-md-2">
            <label>Follow ups >=</label>
            <input type="text" id="followUps" name="followUps" class="form-control form-control-sm" value="0" required>
          </div>
          ...
          <div class="form-group col-md-3">
            <button type="submit" class="btn btn-primary btn-sm">Filter Inquiries</button>
          </div>
        </div>
      </form>
    <?php } ?>
        ...
        $.ajax({
         ...
         success: function(response) {
           ...
           $.each(response,function(key, val) {
             ...
             register = "<a class='btn btn-danger btn-sm' href='<?= base_url(); ?>index.php/enrollments/modify_enroll?studentId="+val.inquiryId+"'><i class='fas fa-pencil-alt'></i></a>&nbsp;"
             register += "<a href='<?= base_url(); ?>index.php/enrollments/print_duplicate?inquiryId="+val.inquiryId+"&courseId="+val.courseId+"' target='_blank' class='btn btn-warning btn-sm'><i class='fas fa-print'></i></a>";
             ...
             t.row.add([
               ...
               val.datetime,
               val.counselor,
               '<div class="inq-status" id="inq_'+val.inquiryId+'"></div>',
             ...
             $.ajax({
               ...
               data: {inq_id:val.inquiryId},
               cache: false,
               success: function(status) {
                 console.log(status);
                 $.each(status,function(k, v) {
                   $('#inq_'+val.inquiryId).append("<span class="+v.status+">"+v.datetime+" - "+v.remarks+"</span>");
                 });
        ...
        success: function(response) {
          ...
          $.each(response,function(key, val) {
            ...
            if(type=='is_registered') {
              ...
              register += "<button type='button' class='btn btn-warning btn-sm btn_enroll' onclick='validate_nic("+val.id+")'><i class='fas fa-user-graduate'></i></button>";
